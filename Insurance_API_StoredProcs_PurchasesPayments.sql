...
-- Idempotent resolver for Safaricom STK-push callbacks (keyed on the
...
-- callback returns Success with o_not_processed=1 so the API doesn't log
-- it as an error). NOTE: keeps the project's executor convention -
-- o_result_code / o_result_message right after the IN params, proc-
-- specific OUT params (o_payment_id, o_not_processed) last.
-- ---------------------------------------------------------------------
...
-- Same semantics and executor-convention ordering as the gateway-
-- reference variant above.
-- ---------------------------------------------------------------------
